...
export const api = {
  ...
  applyInterfaces: (input: { lanDevice: string; wanDevice: string; dialMode: DialMode; logLevel: LogLevel; proxyLocalDns: boolean; expectedRevision: string }) => request<{ ok: boolean; revision: string; running: boolean; interfaces: { lan: string; wan: string }; dialMode: DialMode; logLevel: LogLevel; localDns: StateResponse['localDns']; config?: string }>('apply_interfaces', input),
